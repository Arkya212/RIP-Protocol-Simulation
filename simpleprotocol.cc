...
double
CalculateNodeDistance(Ptr<Node> node1, Ptr<Node> node2)
{
    Vector position1 = GetNodePosition(node1);
    Vector position2 = GetNodePosition(node2);

    double distance = (position2 - position1).GetLength();
    return distance;
}
              ...
              std::map<std::pair<uint32_t, uint32_t>, uint32_t>& weightMap)
{
    for (auto& pair : weightMap)
    {
        ...
        double distance =
            CalculateNodeDistance(NodeList::GetNode(nodeId1), NodeList::GetNode(nodeId2));
        uint32_t weight = static_cast<uint32_t>(distance);

        ripNgRouting.SetInterfaceMetric(NodeList::GetNode(nodeId1), interface1, weight);
        ripNgRouting.SetInterfaceMetric(NodeList::GetNode(nodeId2), interface2, weight);
    }
}
...
// void
// ChangeWeights(RipNgHelper& ripNgRouting, std::map<std::pair<uint32_t, uint32_t>, uint32_t>&
// weightMap)
// {
//     for (auto& pair : weightMap)
//     {
...
//         ripNgRouting.SetInterfaceMetric(NodeList::GetNode(nodeId1), interface1, pair.second + 1);
//         ripNgRouting.SetInterfaceMetric(NodeList::GetNode(nodeId2), interface2, pair.second + 1);
//     }
// }
...
main(int argc, char** argv)
{
    ...
    AnimationInterface anim("result_1.xml");
    ...
    Simulator::Schedule(Seconds(5.0), &ChangeWeights, std::ref(ripNgRouting), std::ref(weightMap));
    Simulator::Schedule(Seconds(10.0), &ChangeWeights, std::ref(ripNgRouting), std::ref(weightMap));
    Simulator::Schedule(Seconds(15.0), &ChangeWeights, std::ref(ripNgRouting), std::ref(weightMap));
    ...
    monitor->SerializeToXmlFile("flowmon-statistics.xml", true, true);

    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter = stats.begin(); iter != stats.end(); ++iter)
{
    Ipv6FlowClassifier::FiveTuple flowKey = classifier->FindFlow(iter->first); // Use iter instead of flow
    FlowMonitor::FlowStats flow = iter->second; // Retrieve flow stats using iter

    std::cout << "Flow: " << flowKey.sourceAddress << " -> " << flowKey.destinationAddress << std::endl;
    ...
    std::cout << "  Mean Packet Size: " << (flow.txBytes / flow.txPackets) << " bytes" << std::endl;
}
